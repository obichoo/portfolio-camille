...
var t = (t2, b2 = {}) => {
  ...
  for (const [t3, e2] of Object.entries(H)) M(e2, A[+t3].degree);
  return Promise.resolve().then(() => P = setTimeout(() => C && (t2.innerHTML = ""), W)), { update(r2) {
    ...
    if (A = p(o2, a2), o2 === G && JSON.stringify(k2) !== JSON.stringify(a2)) for (const [t3, { color: e2 }] of Object.entries(A)) H[+t3].style.setProperty("--bgcolor", e2);
...
var b = (t2) => void 0 === t2;
var k = (t2, e2) => {
  if (!b(t2) && Number.isSafeInteger(t2) && t2 < 0) throw new Error(e2 + " must be a positive integer");
};
var w = ({ particleCount: t2, duration: e2, colors: r2, particleSize: o2, force: a2, stageHeight: i2, stageWidth: n2, particleShape: s2 }) => {
  if (k(t2, "particleCount"), k(e2, "duration"), k(o2, "particleSize"), k(a2, "force"), k(i2, "stageHeight"), k(n2, "stageWidth"), !b(s2) && !/^(mix|circles|rectangles)$/i.test(s2)) throw new Error('particlesShape should be either "mix" or "circles" or "rectangle"');
  if (!b(r2) && !Array.isArray(r2)) throw new Error("colors must be an array of strings");
  if (a2 > 1) throw new Error("force must be within 0 and 1");
};
export {
  t as confetti
};
